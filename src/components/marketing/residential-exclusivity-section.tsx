...
export function ResidentialExclusivitySection() {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: false, amount: 0.3 })

  return (
    <section id="exclusivity" className="relative overflow-hidden bg-[var(--bg-proof)] py-20 md:py-24">
        ...
        >
          ...
          <div className="mb-8 flex flex-wrap items-center justify-center gap-4">
            {["Aldar Properties", "Asteco"].map((name) => (
              <span
                key={name}
                className="rounded-[10px] border border-amber-500/35 px-7 py-3 font-heading text-[14px] font-bold uppercase tracking-wide text-amber-400 transition-colors hover:bg-amber-500/[0.08]"
              >
                {name}
              </span>
            ))}
